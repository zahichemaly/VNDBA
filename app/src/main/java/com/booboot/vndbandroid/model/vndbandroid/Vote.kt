...
object Vote {
    ...
    private fun outOf10(vote: Int): Double = vote / 10.0

    fun toString(vote: Int): String = when {
        vote < 1 -> DEFAULT
        else -> toShortString(vote) + " (" + getName(outOf10(vote)) + ")"
    }

    fun toShortString(vote: Int): String = VOTE_FORMAT.format(outOf10(vote))
    ...
    fun getName(vote: Double): String = when {
        vote >= 10 -> "masterpiece"
        vote >= 9 -> "excellent"
        vote >= 8 -> "very good"
        vote >= 7 -> "good"
    ...
    fun getImage(vote: Double): Int = when {
