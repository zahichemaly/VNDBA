...
@Singleton
open class UserListRepository @Inject constructor(var boxStore: BoxStore, var vndbServer: VNDBServer) : Repository<UserList>() {
    private fun addItemsToDB(items: List<UserList>) {
        boxStore.save(true) { items.map { UserListDao(it, boxStore) } }
    }
