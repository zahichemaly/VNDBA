...
@Singleton
open class UserLabelsRepository @Inject constructor(var boxManager: BoxManager, private var vndbServer: VNDBServer) : Repository<UserLabel>() {
    private fun addItemsToDB(items: List<UserLabel>) {
        boxManager.boxStore.save(true) { items.map { UserLabelDao(it) } }
    }

    override suspend fun getItems(cachePolicy: CachePolicy<Map<Long, UserLabel>>) = cachePolicy
        .fetchFromMemory { items }
        .fetchFromDatabase { boxManager.boxStore.get<UserLabelDao, List<UserLabel>> { it.all.map { userLabelDao -> userLabelDao.toBo() } }.associateBy { it.id } }
        .fetchFromNetwork {
            vndbServer
